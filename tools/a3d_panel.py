...
PANELS = {
    ...
    "esp_lcd_custom": dict(
        label="Your own esp_lcd panel - you supply the bring-up",
        family="custom", targets=("esp32s3", "esp32p4", "esp32c6"),
        components={}, size=(240, 320), swap=False,
        # The only entry whose size is NOT the glass: nobody here knows it, so
        # --viewport is believed rather than overridden. Every other panel
        # overrides it, because a viewport that disagrees with real glass draws
        # a correct picture into the wrong window.
        native=False,
        source="nothing - panel_bring_up() in main/panel.c is yours to write",
    ),
    "spi_st7789": dict(
    ...
    "dsi_jd9365": dict(
        ...
        # A RANGE, not a pin, and this one has already cost this tree a build:
        # jd9365 1.0.6 is for IDF 5.x and 2.0.0 is for 6.x - 5.5.4 still carries
        # esp_lcd_panel_dev_config_t::color_space as a deprecated union member
        # and 6.0 deleted it, so `~1.0.6` fails to COMPILE on 6.0. Neither
        # version is newer-and-better; each is right for one IDF, and the solver
        # is what should choose. Copying a pin copies an assumption about the
        # toolchain.
        components={"waveshare/esp_lcd_jd9365": ">=1.0.6,<3.0.0"},
...
bool a3d_panel_touch(int *x, int *y)
{
    ...
    uint8_t count = 0;
    // esp_lcd_touch 1.2.0 deprecated this for esp_lcd_touch_get_data() and will
    // remove it at 2.0.0. It is kept because get_data() does not exist before
    // 1.2.0 - checked against 1.1.2 - and the manifest above allows either.
    // Silenced rather than left to warn: a warning in code a generator just
    // wrote reads as "this project is already out of date". Change both this
    // and the pin together, before esp_lcd_touch 2.0.0.
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    const bool got = esp_lcd_touch_get_coordinates(s_touch, px, py, strength, &count, 1);
...
// --- MIPI-DSI --------------------------------------------------------------
//
// Commands go over DBI, pixels over DPI. The panel keeps its own framebuffer in
// PSRAM and draw_bitmap copies a tile into it - but with DMA2D enabled, which is
// what `.flags.use_dma2d` and esp_lcd_dpi_panel_enable_dma2d() ask for, THAT
// COPY IS ASYNCHRONOUS. draw_bitmap returns with the 2D-DMA still reading the
// tile, and it rejects the next one outright with ESP_ERR_INVALID_STATE,
// "previous draw operation is not finished". So this waits on the DPI
// completion callback, exactly as the hardware-verified example in this tree
// does. Without the wait the viewer hands the same tile buffer to the next tile
// under a live DMA, and the result looks like a rasterizer bug.
...
// Must be in IRAM: with CONFIG_LCD_DSI_ISR_CACHE_SAFE the registration CHECKS
// it with esp_ptr_in_iram() and refuses otherwise, which is a better failure
// than the cache miss it would be without the check.
static bool IRAM_ATTR on_color_done(esp_lcd_panel_handle_t panel,
                                    esp_lcd_dpi_panel_event_data_t *ev, void *ctx)
{
    (void)panel; (void)ev;
    BaseType_t woken = pdFALSE;
    xSemaphoreGiveFromISR((SemaphoreHandle_t)ctx, &woken);
...
void a3d_panel_send(const uint16_t *src, int x, int y, int w, int h)
{
    ...
    // Drain first rather than trusting the count - see the SPI body for the
    // shape of the bug a stale token causes.
    while (xSemaphoreTake(s_done, 0) == pdTRUE) { }
...
esp_err_t a3d_panel_init(void)
{
    ...
    // The completion signal a3d_panel_send waits on. Registered AFTER the panel
    // exists, because the handle is what carries it.
    esp_lcd_dpi_panel_event_callbacks_t cbs = { .on_color_trans_done = on_color_done };
    ESP_RETURN_ON_ERROR(esp_lcd_dpi_panel_register_event_callbacks(s_panel, &cbs, s_done),
                        TAG, "dpi callbacks");
...
_CUSTOM_BODY = """
// --- your own esp_lcd panel -------------------------------------------------
//
// THIS IS THE ONE OPTION THAT DOES NOT GUESS YOUR WIRING, because it does not
// bring the panel up at all. You do, in panel_bring_up() below, with the code
// that already works on your desk - a vendor example, a BSP, an LVGL port, your
// own. Paste it in and the rest of this file is already correct.
//
// WHAT THE REST OF THIS FILE IS, AND WHY IT IS THE HALF WORTH GENERATING
//
//   The per-panel half is short and you already have it. The half that is the
//   same for every panel is the half that is quietly wrong when hand-written:
//   esp_lcd_panel_draw_bitmap() DOES NOT FINISH WITH YOUR BUFFER BEFORE IT
//   RETURNS on most buses. On SPI, QSPI and I80 the colour transfer is queued
//   and the DMA is still reading it. On MIPI-DSI with DMA2D the copy into the
//   panel's frame buffer is asynchronous, and the NEXT draw_bitmap is rejected
//   with ESP_ERR_INVALID_STATE. a3d hands the same tile buffer to the next tile
//   as soon as a3d_panel_send returns, so a send that does not wait puts half of
//   one tile and half of the next on the glass - and that reads as a renderer
//   bug, which is where the time goes.

// How your bus signals completion. Get this wrong in the safe direction and you
// see a tile that never completes in the log; wrong in the other direction and
// you see tearing.
//
//   PANEL_WAIT_IO    SPI, QSPI, I80 - the panel IO's on_color_trans_done.
//                    panel_bring_up() must return the IO handle for this.
//   PANEL_WAIT_DPI   MIPI-DSI - the DPI panel's on_color_trans_done.
//   PANEL_WAIT_NONE  RGB parallel, and DSI built WITHOUT DMA2D: draw_bitmap
//                    has finished with the buffer when it returns. Only choose
//                    this because you checked, not because it compiles.
#define PANEL_WAIT_IO   0
#define PANEL_WAIT_DPI  1
#define PANEL_WAIT_NONE 2
#define PANEL_WAIT      PANEL_WAIT_IO

#if PANEL_WAIT == PANEL_WAIT_DPI
#include "esp_lcd_mipi_dsi.h"
#endif

static esp_lcd_panel_handle_t    s_panel;
static esp_lcd_panel_io_handle_t s_io;
#if PANEL_WAIT != PANEL_WAIT_NONE
static SemaphoreHandle_t         s_done;
#endif

/* ===========================================================================
 *  FILL THIS IN. It is the only part of this file that is yours.
 * ===========================================================================
 *
 *  Set *out_panel to your esp_lcd_panel_handle_t, initialised and displaying -
 *  reset, init, mirror, swap_xy, invert and disp_on all done. With
 *  PANEL_WAIT_IO, set *out_io to the esp_lcd_panel_io_handle_t you built it on;
 *  a3d registers its completion callback there, which OVERWRITES any callback
 *  already on that IO (esp_lcd logs a warning saying so). Leave *out_io null
 *  for the other two wait modes.
 *
 *  Two things worth doing here rather than paying for them every frame:
 *
 *    - If your controller can be told to read RGB565 little-endian, tell it,
 *      and leave a3d_panel_swap_bytes() false. An ST7789 can (RAMCTRL bit 3,
 *      which esp_lcd spells .data_endian = LCD_RGB_DATA_ENDIAN_LITTLE); at
 *      480x480 the software swap is 11.6 ms a frame.
 *    - Fill the vendor config struct FIELD BY FIELD rather than through its
 *      ESP_LCD_*_CONFIG() macro. Those macros order their designators the way C
 *      allows and C++ rejects, and ESP-IDF 6.0 turned missing initialisers into
 *      errors inside them. This file is C so a macro will compile here, but the
 *      field-by-field form is the one that keeps working across both majors.
 */
static esp_err_t panel_bring_up(esp_lcd_panel_handle_t *out_panel,
                                esp_lcd_panel_io_handle_t *out_io)
{
    (void)out_panel; (void)out_io;

    // ---- delete this block and put your bring-up here ----------------------
    ESP_LOGE(TAG, "panel_bring_up() in main/panel.c has not been written yet.");
    ESP_LOGE(TAG, "That is the one function this generator cannot write for you:");
    ESP_LOGE(TAG, "it is your bus, your GPIOs and your controller. Everything");
    ESP_LOGE(TAG, "else in panel.c - the completion wait, the bus mutex, the");
    ESP_LOGE(TAG, "fill and the touch read - is already done.");
    return ESP_ERR_NOT_SUPPORTED;
    // ------------------------------------------------------------------------
}

#if PANEL_WAIT != PANEL_WAIT_NONE
// In IRAM because it runs from an ISR - and the DPI registration CHECKS that
// with esp_ptr_in_iram() under CONFIG_LCD_DSI_ISR_CACHE_SAFE, so a callback in
// flash is refused there rather than merely being slow.
#if PANEL_WAIT == PANEL_WAIT_IO
static bool IRAM_ATTR on_color_done(esp_lcd_panel_io_handle_t io,
                                    esp_lcd_panel_io_event_data_t *ev, void *ctx)
#else
static bool IRAM_ATTR on_color_done(esp_lcd_panel_handle_t io,
                                    esp_lcd_dpi_panel_event_data_t *ev, void *ctx)
#endif
{
    (void)io; (void)ev;
...
void a3d_panel_send(const uint16_t *src, int x, int y, int w, int h)
{
    ...
    // a failed draw, a timeout that then completed - would satisfy this tile's
    // wait immediately and let the next tile overwrite a buffer the DMA is
    // still reading.
    while (xSemaphoreTake(s_done, 0) == pdTRUE) { }
#endif
...
#if PANEL_WAIT != PANEL_WAIT_NONE
    // A 240x40 tile at 80 MHz is about 1.9 ms. A second is four hundred times
    // that and can only mean the bus has stopped.
    if (xSemaphoreTake(s_done, pdMS_TO_TICKS(1000)) != pdTRUE)
        ESP_LOGE(TAG, "tile at y=%d never completed", y);
#endif
}
...
esp_err_t a3d_panel_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(panel_bring_up(&s_panel, &s_io), TAG, "panel_bring_up");
    ESP_RETURN_ON_FALSE(s_panel, ESP_ERR_INVALID_STATE, TAG,
                        "panel_bring_up returned OK without a panel handle");

#if PANEL_WAIT == PANEL_WAIT_IO
    ESP_RETURN_ON_FALSE(s_io, ESP_ERR_INVALID_STATE, TAG,
                        "PANEL_WAIT_IO needs the panel IO handle from panel_bring_up");
    esp_lcd_panel_io_callbacks_t cbs = { .on_color_trans_done = on_color_done };
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_register_event_callbacks(s_io, &cbs, s_done),
                        TAG, "io callbacks");
#elif PANEL_WAIT == PANEL_WAIT_DPI
    esp_lcd_dpi_panel_event_callbacks_t cbs = { .on_color_trans_done = on_color_done };
    ESP_RETURN_ON_ERROR(esp_lcd_dpi_panel_register_event_callbacks(s_panel, &cbs, s_done),
                        TAG, "dpi callbacks");
#endif
    ...
    a3d_panel_fill(0x0000);
    ESP_RETURN_ON_ERROR(touch_init(), TAG, "touch");
    ESP_LOGI(TAG, "@LABEL@ %dx%d", PANEL_W, PANEL_H);
    return ESP_OK;
}
"""
...
_FAMILY_INCLUDES = {
    ...
    # The user's own bring-up is most often SPI, and these two cost nothing when
    # it is not. Anything else their code needs, they add.
    "custom": '#include "driver/gpio.h"\n#include "driver/spi_master.h"\n',
...
def panel_c(panel: str, touch: str, target: str, width: int, height: int,
    ...
    # The pin warning is the whole point of the header for a catalogue panel and
    # would be a lie on the custom one, which defines no panel pins at all.
    if fam == "custom":
        note = ("// THE PANEL BRING-UP BELOW IS EMPTY ON PURPOSE. panel_bring_up()\n"
                "// is yours: a3d cannot know your bus, your GPIOs or your\n"
                "// controller, and a plausible default driving the wrong GPIO looks\n"
                "// exactly like a dead panel. Everything else in this file - the\n"
                "// completion wait, the bus mutex, fill and touch - is written.")
        if touch != "none":
            note += ("\n//\n// The TOUCH pins further down ARE a guess, and they are "
                     "%s's.\n// Check them too." % t["source"])
    else:
        note = ("// THE PINS BELOW ARE NOT YOUR BOARD'S until you have checked them. "
                "They are\n// %s's.\n"
                "// Nothing in a3d can know your wiring, and a plausible default "
                "driving the\n// wrong GPIO looks exactly like a dead panel."
                % p["source"])

    out = _fill(_COMMON_TOP, LABEL=p["label"], PINS_NOTE=note,
...
def manifest_note(panel: str) -> list:
    """Comment lines for idf_component.yml. Empty for a catalogued panel.

    The custom panel is the only one whose driver a3d cannot name, and a
    manifest that fetches nothing looks finished rather than incomplete - so it
    says out loud that the missing line is the user's.
    """
    if PANELS[panel]["family"] != "custom":
        return []
    return [
        "## Your panel driver goes here. Examples:",
        '##   espressif/esp_lcd_st7796: "1.3.0"',
        '##   espressif/esp_lcd_gc9a01: "2.0.0"',
        "##",
        "## Pin it EXACTLY once you have a build that works on your board. An",
        "## unbounded range is how a vendor driver moves from 2.1.0 to 2.2.0",
        "## with nothing in your project having changed, and the build still",
        "## succeeds - which is why nobody notices. The one case for a range is",
        "## a driver whose major differs per ESP-IDF: the jd9365 above is",
        "## >=1.0.6,<3.0.0 because 1.x is for IDF 5 and 2.x for IDF 6.",
        "##",
        "## Nothing is fetched for a controller esp_lcd itself carries (ST7789,",
        "## NT35510), so an empty list here is not necessarily wrong.",
    ]
...
def sdkconfig_extra(panel: str, target: str) -> list:
    ...
    if p["family"] == "custom":
        return [
            "",
            "# No bus-specific configuration: a3d does not know which bus",
            "# panel_bring_up() will use. If yours needs PSRAM speed, cache or",
            "# DMA settings - a MIPI-DSI panel keeps a framebuffer in PSRAM, an",
            "# RGB one needs bounce buffers - add them here.",
        ]
    if p["family"] == "qspi":
        return [
            "",
